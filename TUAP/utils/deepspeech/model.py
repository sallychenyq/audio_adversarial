...
from utils.deepspeech.train_config import SpectConfig, BiDirectionalConfig, OptimConfig, AdamConfig, \
    SGDConfig
from utils.deepspeech.decoder import GreedyDecoder
...
class DeepSpeech(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        #print("After collapsing and transposing:", x.size())

        for i, rnn in enumerate(self.rnns):
            x = rnn(x, output_lengths)
            #print("RNN layer", i + 1, "output:", x.size())
            # 获取声谱图数据
            #seq_len, _, feature_dim = x.size()
            # 将输出张量转换为 numpy 数组
            output_np = x.cpu().squeeze().detach().numpy()
            # 创建时间步和特征维度的坐标网格
            feature_dim = output_np.shape[1]
# 绘制热力图
            last_timestep_features = output_np[-1,:]
            feature_indices = range(feature_dim)
            #print(feature_indices)
            #plt.plot(feature_indices, last_timestep_features)
            #plt.colorbar()
            #plt.xlabel('Feature Index')
            #plt.ylabel('Feature Value')
            #plt.title('Last Timestep Features')
            #plt.savefig("rnn_layer{}_output.png".format(i + 1))
            #plt.close()
        if not self.bidirectional:
            x = self.lookahead(x)
            #print("Lookahead output:", x.size())

        x = self.fc(x)
        x = x.transpose(0, 1)
        #print("FC output:", x.size())

        x = self.inference_softmax(x)
        #print("Output after inference softmax:", x.size())
