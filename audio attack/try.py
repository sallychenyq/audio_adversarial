...
def audio_scatter(is_shuffle=False):
    g2p = G2p()
    dev_url = "dev-clean"
...
def generate_data(num, alpha, is_shuffle=False):
    ...
    waveform, sample_rate = torchaudio.load("./audiodata/poweroff.wav") #len=2?
    ...
    permut_list = []
    for res in itertools.permutations(phoneme_list):
       permut_list.append(list(res))
    # s_num = len(phoneme_list)
    # s_len = sound.shape[1] / 16000
    # density = s_num / s_len
    print(permut_list[0])
    # dev_url = "dev-clean" #2703audios?
    # pipeline_dataset = torchaudio.datasets.LIBRISPEECH("./dataset", url=dev_url, download=False)
    # pipeline_loader = data.DataLoader(dataset=pipeline_dataset,
    #                                   batch_size=1,
    #                                   shuffle=is_shuffle)
    ...
    #     sound = y[0][0]
    #     label = y[2][0]
    #     #print(y) [tensor([[[-5.7983e-04, -8.5449e-04, -9.1553e-04,  ...,  6.1035e-05,
    # #      -9.1553e-05,  3.0518e-04]]]), tensor([16000]), ('ALL IDEALISATION MAKES LIFE POORER',), tensor([777]), tensor([126732]), tensor([0])]
